...
export default function Tooltip({ label, children, side = 'top', className = '' }) {
  ...
  return (
    <span className={`group/tip relative inline-flex ${className}`}>
      ...
      <span
        role="tooltip"
        className={`pointer-events-none absolute left-1/2 z-40 -translate-x-1/2 scale-95 whitespace-nowrap rounded-md border border-border bg-card px-2 py-1 text-xs font-normal text-foreground opacity-0 shadow-popover transition-[opacity,transform] duration-150 group-hover/tip:scale-100 group-hover/tip:opacity-100 group-focus-within/tip:scale-100 group-focus-within/tip:opacity-100 ${place}`}
      >
        {label}
      </span>
    </span>
